            ...
            {players.map((player, index) => (
                <div
                    ...
                    className={
                        ...
                        (gameOver && player.winner ? " my-1" : "") +
                        (!gameOver && playerTurn === index ? " my-2" : "")
                    }
                    style={
                        gameOver && player.winner ?
                        {border: `2px solid var(--player${index + 1}-bg-color)`,
                        animation: "pulse 1s infinite"
                        } :
                        !gameOver && playerTurn === index ?
                        {border: `2px solid var(--player${index + 1}-bg-color)`}:{}
                    }
                >
                    <div className="flex items-center">
                        <svg className={"trophy" + (player.winner ? "" : " trophy-hidden")}
                        ...
                        <span style={{color:`var(--player${index+1}-color)`}}>{player.name}</span>
                    </div>
